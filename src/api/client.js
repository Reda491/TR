...
function mapMotorcycle(row) {
  return {
    ...
    price_per_day: row.dailyRate ?? 0,
    price_1_3_days: row.dailyRate,
    price_3_5_days: row.dailyRate ? Math.max(1, row.dailyRate - 5) : undefined,
    price_5plus_days: row.dailyRate ? Math.max(1, row.dailyRate - 10) : undefined,
    image_url: row.image,
